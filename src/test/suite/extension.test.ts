...
function sampleFileUri(fileName: string) {
  return Uri.file(
    path.join(__dirname, "..", "..", "..", "src", "test", "samples", fileName)
  );
}

function openFile(fileName: string, doneCallback: (Error: Error) => void) {
  return workspace.openTextDocument(sampleFileUri(fileName)).then(
    function(doc) {
      return window.showTextDocument(doc).then(
        () => {
          return commands.executeCommand("cursorTop");
        },
...
function getActiveDocument(): TextDocument {
...
describe("Integration tests", function() {
  ...
  describe("commands", function() {
    it("work in ruby files", function(done) {
      ...
      openFile("sample.rb", done).then(
        function() {
          commands.executeCommand("seeing-is-believing.toggle-marks").then(
            function() {
    ...
    extensionCommands.forEach(function(command) {
      it(`won't run seeing-is-believing.${command} in other files`, function(done) {
        openFile("sample.js", done).then(
          function() {
            commands.executeCommand(`seeing-is-believing.${command}`).then(
              function() {
                done("Should have failed to execute command");
              },
              function(error) {
                expect(error).to.equal(
                  "Seeing is Believing can only process Ruby files"
                );
                done();
              }
            );
  ...
  describe("annotations", function() {
    describe("adding", function() {
      ...
      context("some lines are annotated", function() {
        it("adds the annotation mark to unmarked lines in the same selection group", function(done) {
          ...
          commands.executeCommand("cursorTop").then(
            function() {
              commands.executeCommand("cursorDown").then(
                function() {
                  commands.executeCommand("cursorDown").then(
                    function() {
                      commands
                        ...
                        .then(
                          function() {
                            ...
                            expect(document.lineAt(1).text).to.equal(
                              'last_name = "Simone"'
                            );
                            expect(document.lineAt(2).text).to.equal(
                              'dob = "1/23/80"  # =>'
                            );

                            commands.executeCommand("cursorUpSelect").then(
                              function() {
                                commands.executeCommand("cursorUpSelect").then(
                                  function() {
                                    commands
              ...
              commands.executeCommand("seeing-is-believing.toggle-marks").then(
                function() {
                  ...
                  commands
                    ...
                    .then(
                      function() {
                        commands.executeCommand("cursorRightSelect").then(
                          function() {
                            commands
                              .executeCommand(
                                "seeing-is-believing.toggle-marks"
      ...
      context("adds the annotation mark to multiple lines", function() {
        it("aligns marks in the same selection group", function(done) {
          ...
          commands.executeCommand("cursorTop").then(
            function() {
              commands.executeCommand("cursorDownSelect").then(
                function() {
                  commands.executeCommand("cursorDownSelect").then(
                    function() {
                      commands
        ...
        it("does not align different selection groups", function(done) {
          ...
          commands.executeCommand("cursorTop").then(
            function() {
              commands.executeCommand("editor.action.insertCursorBelow").then(
                function() {
                  commands
                    .executeCommand("editor.action.insertCursorBelow")
                    .then(
    ...
    describe("removing", function() {
      beforeEach(function(done) {
        openFile("sample-marked.rb", done).then(
          function() {
            done();
          },
          function() {
  ...
  describe("require relative files", function() {
    beforeEach(function(done) {
      openFile("requirer.rb", done).then(
        function() {
          done();
        },
        function() {
    ...
    it("works properly", function(done) {
      ...
      commands.executeCommand("seeing-is-believing.run").then(
        function() {
  ...
  describe("run and clean", function() {
    ...
    it("displays an error if the executable is unavailable", function(done) {
      ...
      commands.executeCommand("seeing-is-believing.run").then(
        ...
        function() {
          expect(
            spy.calledWith(
              `Command 'fake_command' does not exist. Is it installed?`
            )
          ).to.equal(true);
          spy.restore();
          SeeingIsBelieving.command = cachedCommand;
    ...
    it("updates the document text and then cleans it", function(done) {
      ...
      const lastLineWithText = () =>
        linesWithText()[linesWithText().length - 1];

      expect(lastLineWithText()).to.eq(
        'puts "My name is #{first_name} and I was born #{dob}"'
      );

      commands.executeCommand("seeing-is-believing.run").then(
        function() {
          expect(lastLineWithText()).to.eq(
            "# >> My name is Jordan and I was born 1/23/80"
          );

          commands.executeCommand("seeing-is-believing.clean").then(
            function() {
              expect(lastLineWithText()).to.eq(
                'puts "My name is #{first_name} and I was born #{dob}"'
              );
    ...
    context("xmpfilter-style is disabled if none marked", function() {
      it("annotates the whole document if no lines are marked", function(done) {
        ...
        commands.executeCommand("seeing-is-believing.run").then(
          function() {
            expect(document.lineAt(0).text).to.eq(
              'first_name = "Jordan"  # => "Jordan"'
            );
            expect(document.lineAt(1).text).to.eq(
              'last_name = "Simone"   # => "Simone"'
            );
            expect(document.lineAt(2).text).to.eq(
              'dob = "1/23/80"        # => "1/23/80"'
            );
            expect(document.lineAt(3).text).to.eq("");
            expect(document.lineAt(4).text).to.eq(
              'puts "My name is #{first_name} and I was born #{dob}"  # => nil'
            );
            expect(document.lineAt(5).text).to.eq("");
            expect(document.lineAt(6).text).to.eq(
              "# >> My name is Jordan and I was born 1/23/80"
            );
