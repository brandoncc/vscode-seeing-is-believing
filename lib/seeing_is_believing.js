...
const SeeingIsBelieving = {
  ...
  run: function() {
    ...
    const proc = spawn(this.command, this.args.run);
    let stdout = '';
    let stderr = '';

    return new Promise((res, rej) => {
      proc.on('error', () => {
        window.showErrorMessage(`Command '${this.command}' does not exist. Is it installed?`);
        rej(`Command '${this.command}' does not exist. Is it installed?`);
        return;
      });
